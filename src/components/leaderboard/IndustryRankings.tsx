...
import { Briefcase, Crown, Star, Award, ChevronLeft, ChevronRight } from 'lucide-react';
import { cn } from '@/lib/utils';

interface IndustryRankingsProps {
  industryRankings: Array<{name: string, score: number, companies?: number, students?: number, logo?: string}>;
  currentPage: number;
  totalPages: number;
...
const IndustryRankings = ({ 
  ...
  navigateToPage,
  generatePageNumbers
}: IndustryRankingsProps) => {
  // Ensure we have data to display
  if (!industryRankings || industryRankings.length === 0) {
    console.log("No industry rankings data available");
    return (
      <div className="flex items-center justify-center py-12">
        <p className="text-muted-foreground">No industry rankings available</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="hidden md:flex justify-center items-end space-x-6 py-8">
        {industryRankings.slice(0, 3).map((industry, index) => {
          ...
          return (
            <div key={`industry-${index}`} className="flex flex-col items-center">
              <div className={cn(
              ...
              <span className="font-bold text-center max-w-[160px] text-center">{industry.name}</span>
              <span className="text-sm text-muted-foreground">{industry.score} pts</span>
              ...
              >
                <div className="flex flex-col items-center text-white">
                  ...
                  <span className="text-sm font-medium">{industry.companies || 0} companies</span>
                </div>
              </div>
            </div>
          );
      ...
      <Card className="overflow-hidden border-2 shadow-md">
        <CardHeader className="pb-2 bg-gradient-to-r from-background via-dopamine-purple/5 to-dopamine-pink/5">
          ...
          <CardDescription>
            Top performing industries based on company and student achievements
          </CardDescription>
        </CardHeader>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      {industry.companies || 0} companies, {industry.students || 0} students
                    </div>
                  </div>
                </div>
                
                <div className="flex items-center gap-4">
                  <div className="font-semibold text-lg">{industry.score}</div>
